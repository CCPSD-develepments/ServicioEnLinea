	...
	var init = function(){
		...
		if (type == 'box'){
			...
			if (options.deleteButton){
                bit.addClass(typeprefix + '-deletable' + ' upper-case');
                close = $('<a href="javascript:return false;" class="' + typeprefix +'-deletebutton" />').click(remove).appendTo(bit);
			}
			bit.children().click(function(e){ e.stopPropagation(); e.preventDefault(); });
		} else {
            element = $('<input type="text" class="' + typeprefix +'-input upper-case" autocomplete="off" />').val(self.value ? self.value[1] : '').appendTo(bit);
