//------------------------------------------------------------------------------
// <auto-generated>
//     This code was generated from a template.
//
//     Changes to this file may cause incorrect behavior and will be lost if
//     the code is regenerated.
// </auto-generated>
//------------------------------------------------------------------------------

using System;
using System.Collections;
using System.Collections.Generic;
using System.Collections.ObjectModel;
using System.Collections.Specialized;

namespace CamaraComercio.DataAccess.EF.OficinaVirtual
{
    [Serializable]
    public partial class TransaccionesDocDescargas
    {
        #region Property Metadata
    	
    	public struct PropColumns
    	{
    			public static string DescargaId = @"DescargaId";	
    	public static string TransaccionId = @"TransaccionId";	
    	public static string DocContent = @"DocContent";	
    	public static string FechaDescarga = @"FechaDescarga";	
    	public static string UserName = @"UserName";	
    	public static string NombreSocialOrComment = @"NombreSocialOrComment";	
     
    	}
        #endregion

        #region Primitive Properties
    
        public virtual int DescargaId
        {
            get;
            set;
        }
    
        public virtual int TransaccionId
        {
            get { return _transaccionId; }
            set
            {
                if (_transaccionId != value)
                {
                    if (Transacciones != null && Transacciones.TransaccionId != value)
                    {
                        Transacciones = null;
                    }
                    _transaccionId = value;
                }
            }
        }
        private int _transaccionId;
    
        public virtual byte[] DocContent
        {
            get;
            set;
        }
    
        public virtual Nullable<System.DateTime> FechaDescarga
        {
            get;
            set;
        }
    
        public virtual string UserName
        {
            get;
            set;
        }
    
        public virtual string NombreSocialOrComment
        {
            get;
            set;
        }

        #endregion

        #region Navigation Properties
    
        public virtual Transacciones Transacciones
        {
            get { return _transacciones; }
            set
            {
                if (!ReferenceEquals(_transacciones, value))
                {
                    var previousValue = _transacciones;
                    _transacciones = value;
                    FixupTransacciones(previousValue);
                }
            }
        }
        private Transacciones _transacciones;

        #endregion

        #region Association Fixup
    
        private void FixupTransacciones(Transacciones previousValue)
        {
            if (previousValue != null && previousValue.TransaccionesDocDescargas.Contains(this))
            {
                previousValue.TransaccionesDocDescargas.Remove(this);
            }
    
            if (Transacciones != null)
            {
                if (!Transacciones.TransaccionesDocDescargas.Contains(this))
                {
                    Transacciones.TransaccionesDocDescargas.Add(this);
                }
                if (TransaccionId != Transacciones.TransaccionId)
                {
                    TransaccionId = Transacciones.TransaccionId;
                }
            }
        }

        #endregion

    }
}
