...
namespace CamaraComercio.DataAccess.EF.OficinaVirtual
{
    ...
    public class TransaccionesController
    {
        ...
        public List<Transacciones> ObtenerTransaccionesProblema(string padreUser)
        {
            ...
            //  var dbUsers = new Membership.CamaraWebsiteAccountsEntities();
            using (var dbUsers = new Membership.CamaraWebsiteAccountsEntities())
            {
                ...
                //            .Select(c => c.UserName.ToLower()).ToList();
                 ...
                 var uHijosLinq = from trU in dbUsers.ViewProfileProperties
                               where (trU.UsuarioPadre == padreUser)
                               select trU.UserName.ToLower();

                var uHijos = uHijosLinq.ToList();
        ...
        //--buscar data:
        public List<Transacciones> GetTransaccionesByFilter(string userId, string BuscarPor,
            int ValorBuscar = 0, string _nombreSocial="", DateTime? fDesde = null, DateTime? fHasta = null, string mTipoSociedad="")
        {
            DateTime _fDesde;
            DateTime _fHasta;
            ...
            if (BuscarPor == "Fecha")
            {
                 _fDesde = Convert.ToDateTime(fDesde.Value.Date); //mm/dd/yyyy
                 _fHasta = Convert.ToDateTime(fHasta.Value.Date);//mm/dd/yyyy               

                string fechaInicial = fDesde.Value.Date.ToString("yyyy/MM/dd");
                string fechaFinal = fHasta.Value.Date.ToString("yyyy/MM/dd");
                              

                DateTime.TryParse(fechaInicial, out fecha1);
                DateTime.TryParse(fechaFinal, out fecha2);
            }
            ...
            using (var dbUsers = new Membership.CamaraWebsiteAccountsEntities())
            {
                ...
                //var UsuarioTurnoTotal = db.Transacciones.Where(a => a.UserName.Equals((string)userId))
                //      .Where(a => a.Fecha >= fecha1.Date && a.Fecha <= (fecha2.Date) && (!a.SubTransaccionId.HasValue)).ToList();

                var transac = from tr in db.Transacciones
                              where (tr.UserName == userId || uHijos.Contains(tr.UserName)) && (!tr.SubTransaccionId.HasValue)
                              select tr;
                if (BuscarPor == "Todas")
                {
                    transacciones.AddRange(transac.ToList().OrderByDescending(a => a.Fecha));
                }
                else
                 if (BuscarPor == "RM")
                {
                    if (mTipoSociedad == "Sociedad")
                    {
                        transacciones.AddRange(transac.Where(c => c.NumeroRegistro == ValorBuscar && c.TipoSociedadId != 7).ToList().OrderByDescending(a => a.Fecha));
                    }
                    else
                        if (mTipoSociedad == "Persona")
                    {
                        transacciones.AddRange(transac.Where(c => c.NumeroRegistro == ValorBuscar && c.TipoSociedadId == 7).ToList().OrderByDescending(a => a.Fecha));
                    }
                 ...
                 if (BuscarPor == "Estado")
                {
                    ...
                    transacciones.AddRange(transac.Where(c => c.EstatusTransId == ValorBuscar).ToList().OrderByDescending(a => a.Fecha));
                }
                else
                 if (BuscarPor == "SolicitudId")
                {

                    transacciones.AddRange(transac.Where(c => c.TransaccionId == ValorBuscar).ToList().OrderByDescending(a => a.Fecha));
                }
                else
                 if (BuscarPor == "Persona")
                {

                    transacciones.AddRange(transac.Where(c => c.NombreSocialPersona.Contains(_nombreSocial) && c.TipoSociedadId == 7).ToList().OrderByDescending(a => a.Fecha));
                }
                else
                 if (BuscarPor == "Sociedad")
                {
                 ...
                 if (BuscarPor == "Fecha")
                {
                    ...
                    var fec1 = DateTime.Parse(fecha1.ToString());
                    var fec2 = DateTime.Parse(fecha2.ToString());

                    transacciones.AddRange(transac.Where(c => c.Fecha >= fec1 && c.Fecha <= fecha2).ToList().OrderByDescending(a => a.Fecha));
                }
        ...
        //--buscar data:
        public List<Transacciones> GetTransaccionesByFilterPaging(string userId, string BuscarPor,
            int ValorBuscar = 0)
        {
            ...
            //var transacPaging = (from c in transacciones
            //                   //  where (c.UserName == userId || uHijos.Contains(c.UserName) || c.UserName == uPadre) && (!c.SubTransaccionId.HasValue)
            //               orderby c.Fecha descending
            //               select c).Skip(startRowIndex).Take(maximumRows).ToList();
